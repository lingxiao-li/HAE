...
def run():
    ...
    for input_batch in tqdm(dataloader):
        ...
        for i in range(opts.test_batch_size):
            ...
            embeds.append(result_batch[2][i].cpu().numpy())
            euc_embeds.append(result_batch[-1][i].cpu().numpy())
            labels_save.append(label[i].cpu().numpy())
    ...
    results_path = os.path.join(opts.exp_dir, 'embed')
    image_path = os.path.join(opts.exp_dir, 'image_path')
    label_path = os.path.join(opts.exp_dir, 'label')
    euc_path = os.path.join(opts.exp_dir, 'euc_embed')
    result_str = 'Runtime {:.4f}+-{:.4f}'.format(np.mean(global_time), np.std(global_time))

    #print(results)
    print('now saving')
    torch.save(embeds, results_path)
    torch.save(image_paths, image_path)
    torch.save(labels_save, label_path)
    torch.save(euc_embeds, euc_path)
